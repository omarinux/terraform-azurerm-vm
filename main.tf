...
resource "local_file" "ansible_inventory_linux" {
  ...
  content = templatefile("${path.module}/inventory_linux.tmpl",
    {
     linux_vms_name = azurerm_virtual_machine.vm_linux.*.name,
     linux_vms_ip = azurerm_virtual_machine.vm_linux.*.public_ip_address
    }
  )
...
resource "local_file" "ansible_inventory_windows" {
  ...
  content = templatefile("${path.module}/inventory_windows.tmpl",
    {
     windows_vms_name = azurerm_virtual_machine.vm_windows.*.name,
     windows_vms_ip = azurerm_virtual_machine.vm_windows.*.public_ip_address
    }
  )
